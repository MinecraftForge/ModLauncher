...
class TransformStoreTests {
    ...
    void testGatherTransformersNormally() throws Exception {
        if (!ModLauncherTest.isTransformed()) {
            ...
            TransformStore store = UnsafeHacksUtil.getInternalState(Launcher.INSTANCE, "transformStore");
            EnumMap<LabelType, TransformList<?>> transformers = UnsafeHacksUtil.getInternalState(store, "transformers");
            Set<String> targettedClasses = UnsafeHacksUtil.getInternalState(store, "classNeedsTransforming");
    ...
    private static <T> Map<TransformTargetLabel, List<ITransformer<T>>> getTransformers(TransformList<T> list) {
        return UnsafeHacksUtil.getInternalState(list, "transformers");
    }

    private static <T> ITransformer<T> unwrap(ITransformer<T> wrapper) {
        return UnsafeHacksUtil.getInternalState(wrapper, "wrapped");
    }
    ...
    private Transformers getTransformers() throws Exception {
        ...
        final ThreadLocal<?> thread = UnsafeHacksUtil.getInternalState(cls, "TRANSFORMERS");
        assertNotNull(thread, "Could not find ThreadLocal TRANSFORMERS in " + cls);
        var inst = thread.get();
        return new Transformers(
            UnsafeHacksUtil.getInternalState(inst, "cls"),
            UnsafeHacksUtil.getInternalState(inst, "field"),
            UnsafeHacksUtil.getInternalState(inst, "method")
        );
    }
}
